...
export const getAllNotificationsByUser = (userId) => {
  ...
  const NotifIdList = normalizedData.result.filter(
    (notifId) => notifications[notifId].author == userId
  );
  NotifIdList.forEach((id) =>
    userNotifs.push(messages[notifications[id].context])
  );
  return userNotifs;
};
